...
def process_image_with_area_analysis(image_path, processed_path, region_info=None):
    ...
    if yolo_infer.model is None:
        print(f"[ERROR] YOLO model not loaded")
        return {"error": "YOLO model not loaded. Please check the model path and restart the server."}
    ...
    return {
        ...
        "selected_area_sqm": float(round(selected_area_sqm, 2)),
        "detected_people_count": int(detected_people_count),
        "estimated_max_capacity": int(estimated_max_capacity),
        "current_density": float(round(current_density, 3)),
        "current_density_percentage": float(round(current_density_percentage, 1)),
...
def process_image_batch(image_path, processed_path, personal_space, scale_factor):
    ...
    return {
        "detected_people_count": int(people_count),
        "selected_area_sqm": float(round(selected_area_sqm, 2)),
        "estimated_max_capacity": int(estimated_max_capacity),
        "current_density": float(round(current_density, 3)),
        "current_density_percentage": float(round(current_density_percentage, 1)),
