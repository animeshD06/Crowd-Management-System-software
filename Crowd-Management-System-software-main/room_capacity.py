...
def estimate_room_capacity(image_path: str) -> dict:
    ...
    if labels.max() == 0:
        print("[WARN] No floor detected in the image, using fallback estimation")
        # Return a reasonable default based on image size
        img_area = img.shape[0] * img.shape[1]
        estimated_area = img_area / 25000  # Default pixels per sqm
        return {
            "estimated_area": estimated_area,
            "maximum_people": int(estimated_area * 2),
            "depth_map_path": None,
            "floor_overlay_path": None
        }
