    ...
    def estimate_area_from_detections(self, width, height, detections, image_path=None):
        ...
        if image_path:
            try:
                ...
                if result and "estimated_area" in result:
                    area = result["estimated_area"]
                    if area > 0:
                        print(f"[DEBUG] MiDaS estimated area: {area:.2f} sqm")
                        return area
                    else:
                        print(f"[WARN] MiDaS returned invalid area: {area}, falling back")
                else:
                    print(f"[WARN] MiDaS returned invalid result, falling back")
            except Exception as e:
